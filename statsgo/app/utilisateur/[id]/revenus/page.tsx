...
const CreerDepenses = () => {
  ...
  const onSubmit = async (data: Schema) => {
    try {
      const response = await axios.post(`/api/utilisateurs/${SessionId}/revenus`, data);
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center">
      <div className="bg-white p-8 rounded-lg shadow-lg w-96">
        <h2 className="text-2xl font-bold text-center mb-6">Créer un revenu</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          >
            {isSubmitting ? "En cours" : "Creer un revenu "}
          </button>
        </form>
      </div>
    </div>
